...
app = Flask(__name__)
app.secret_key = 'village-rwenzori-sidebar-pro-2026'
BOUTIQUE = {
...
STOCK = {"RIZ": {"qte": 50, "prix": 3000},"SUCRE": {"qte": 30, "prix": 2500}}
VENTES = []
PIN_GERANT = "1234"
PIN_VENDEUR = "0000"
def is_expired():
    ...
    EXPIRE_HTML = '''
<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Expire</title>
...
.num{font-size:18px;font-weight:bold;color:#0b3d91}.pay-box{background:#f8f9fa;border:1px dashed #0b3d91;padding:14px;border-radius:10px;margin:15px 0;text-align:left}</style>
</head><body><div class="card"><h2 style="color:#dc3545">ABONNEMENT EXPIRE</h2><p>{{b.nom}} expire {{b.expire}}</p>
<div class="pay-box"><b>Pour reactiver 1 MOIS:</b><br><br>M-Pesa: <span class="num">{{b.mpesa}}</span><br>Orange: <span class="num">{{b.orange}}</span><br><br>Montant 10$</div></div></body></html>
'''
LOGIN_HTML = '''
...
.card{background:#fff;padding:32px;border-radius:18px;width:92%;max-width:390px;text-align:center}
...
{% if error %}<p style="color:red">{{error}}</p>{% endif %}<div class="badge">ABONNEMENT ACTIF</div><div class="pay">Renouvellement:<br>M-Pesa <b>{{b.mpesa}}</b><br>Orange <b>{{b.orange}}</b></div></div></body></html>
'''
DASH_PRO_HTML = '''
<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>TANGA STOCK PRO</title>
<style>*{box-sizing:border-box}body{margin:0;font-family:Arial;background:#f1f4f9;display:flex;min-height:100vh}
.sidebar{width:220px;background:#1e293b;color:#fff;padding:15px;display:flex;flex-direction:column}
.badge-role{background:#facc15;color:#000;padding:5px 10px;border-radius:12px;font-size:11px;font-weight:bold;display:inline-block;margin-bottom:15px}
.menu a{display:block;color:#cbd5e1;text-decoration:none;padding:10px 12px;border-radius:8px;margin-bottom:6px;font-size:13px}
...
<script>function calcTotal(){let q=document.getElementById('qte').value;let p=document.getElementById('prix_fixe').value;let t=q*p;document.getElementById('total_auto').innerText=t+' FC - AUTO';}</script>
</head><body>
<div class="sidebar"><h2>TANGA STOCK PRO</h2><div class="badge-role">{{role}}</div>
<div class="menu"><a href="/dashboard" class="active">Vente</a><a href="/dashboard?view=inventaire">Inventaire {% if role!='Gérant' %}🔒{% endif %}</a><a href="/dashboard?view=factures">Factures</a><a href="/logout">Quitter (Changer de role)</a></div>
<div style="margin-top:auto;font-size:10px;color:#94a3b8">M-Pesa {{b.mpesa}}<br>Orange {{b.orange}}<br>Expire {{b.expire}}</div></div>
<div class="main"><div class="header">{{b.nom}} - Mode: {{role|lower}} - Prix FIXE par Gerant | Calcul AUTO Qte*Prix</div>
{% if view=='vente' or not view %}
<div class="card"><h3 style="margin:0 0 10px">Vente - Prix BLOQUE, Calcul AUTO</h3>
<div class="blue-box">Mode <b>{{role}}</b> - Le prix est fixe par le gerant. Tu ne peux pas le changer. Total = Qte x Prix Fixe.</div>
<form method="POST" action="/sell_pro"><div class="input-row">
<div class="input-group" style="flex:2"><label>Client:</label><input name="client" value="MOISE MWENGE" required></div><div class="input-group"><label>Tel</label><input name="tel" placeholder="099..."></div></div>
...
{% for nom,data in stock.items() %}<option value="{{nom}}" data-prix="{{data.prix}}">{{nom}} - {{data.prix}} FC (Stock:{{data.qte}}) - PRIX FIXE</option>{% endfor %}</select></div>
<div class="input-group"><label>Qte</label><input type="number" id="qte" name="qte" value="1" min="1" oninput="calcTotal()" required></div>
<div class="input-group"><label>Total AUTO (Qte*Prix)</label><div class="total-auto" id="total_auto">3000 FC - AUTO</div><input type="hidden" id="prix_fixe" value="3000"></div></div>
<button class="btn-vendre">VENDRE - Calcul Auto vendeur</button></form></div>
{% endif %}
{% if view=='inventaire' %}
...
'''
FACTURE_HTML = '''
<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Facture {{v.id}}</title>
...
def dashboard():
    ...
    return render_template_string(DASH_PRO_HTML, b=BOUTIQUE, role=session['role'], stock=STOCK, ventes=VENTES, view=view)
@app.route('/stock')
def old_stock():
    return redirect('/dashboard')
